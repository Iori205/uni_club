  ...
  return (
    <section className="bg-background">
      <div className="mx-auto max-w-5xl px-5 pt-6 pb-14 lg:px-8 lg:pt-6 lg:pb-16">
        <Link
          href="/#departments"
          className="inline-flex items-center gap-2 text-sm font-medium text-muted-foreground transition-colors hover:text-primary md:absolute  md:left-14"
        >
        ...
        {leads.length > 0 && (
          <div className="mt-9 grid gap-5 sm:grid-cols-2">
            {leads.map((member) => (
              <article
                key={member.name}
                className="flex items-center gap-4 rounded-2xl border border-border bg-card p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md"
              >
                <SafeImage
                  src={member.image}
